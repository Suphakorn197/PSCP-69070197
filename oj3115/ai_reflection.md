...
| OJ submission ID, if submitted | 632836 |
| OJ status | Pass |
...
```text
ถามเรื่องการใช้list,ตอนปริ้นต้องทำยังไงแล้วก็ให้สอนว่าlist(map(int, input().split()))คืออะไร
```
...
```text
แชทช่วยชี้ให้เห็นบัคในโค้ดเดิมของผมเช่น การเผลอใส่คำสั่ง break เร็วเกินไปทำให้รับค่าได้แค่ร้านเดียวและการลืมสร้างตัวแปรมารับค่าในบรรทัดสุดท้าย
```
...
```text
แชมช่วยให้สังเกตว่าโจทย์ข้อนี้ไม่สามารถใช้for loopซ้อนกันเพื่อเช็คทีละร้านได้เพราะข้อมูลมีเยอะถึงแสนตั จะทำให้โปรแกรมรันช้าและติดTime Outแชทเลยแนะนำเทคนิคให้สร้างListเวลาขึ้นมา 1,441 ช่องแล้วใช้การทำเครื่องหมาย +1 ตอนร้านเปิดและ -1ตอนร้านปิดแทนนอกจากนี้ยังช่วยชี้ให้เห็นบัคตอนที่ผมตั้งใจจะรับข้อมูลบรรทัดสุดท้ายแต่ลืมสร้างตัวแปรมารับค่าด้วย
```
...
```text
ได้รู้ว่า list(map(int, input().split())) คืออะไร มันก็คือการที่เราเอาตัวเลขที่แบ่งคั่นด้วยช่องว่างมาใส่กล่องlist
ได้รู้ว่าการที่เราใส่ดอกจันทร์หน้าanswersตอนปริ้นจะทำให้ตอนoutputออกมาไม่มีกล่องlist
ได้รู้ว่าการที่เราเอากล่องlistใส่0 แล้วคูณกับตัวเลขที่ต้องการเป็นเทคนิคการสร้างช่องlistว่าง เตรียมรอเอาไว้เพื่อให้เรานำไปใช้เป็นตารางบันทึกข้อมูลในภายหลังได้
ได้เรียนรู้ว่าแทนที่เราจะสั่งให้คอมพิวเตอร์บวกเลขทุกๆนาทีที่ร้านเปิดเราสามารถทำเครื่องหมายแค่ตอนเริ่มต้น time[start] + 1และตอนสิ้นสุดtime[end] - 1แล้วค่อยนำไปบวกสะสมยอดทีเดียววิธีนี้ช่วยประหยัดเวลาการทำงานของโปรแกรมได้เยอะเลยครับ
```
