...
จำนวนเต้มn
จำนวนจริงหรือเต็มก็ได้ ต้องใช้float
Output:
ยอดสุทธิหลังหักส่วนลด (ทศนิยม 2 ตําแหน่ง)
Constraints:
ต้องมีการใช้floatและ.upper()ด้วย
```
...
```text
Step 1:รับค่าตัวอักษรและจำนวนเต็มเข้ามา
Step 2:กำหนดtotalและprice=0
Step 3:เข้าfor loop
Step 4:รับค่าราคาสินค้า n บรรทัดต้องรับเป็นfloat
Step5:กำหนดtotal = total + price_stock
Step6:ใช้ if elif elseเช้ค
Step7:ใช้ round half up
Step8:printใช้f-string .2fด้วย
```
...
```text
วิธีสุดท้ายเหมือนกับแผนแรก คือรับค่าตัวอักษรและจํานวนรายการสินค้าที่ซื้อเข้ามาก่อน จากนั้นก็กำหนดให้pricreและtotal = 0 เพราะเราต้องมีการคำนวณจากนั้นก็เข้าfor loop แล้วรับค่าราคาสินค้า n บรรทัดเข้ามาจากนั้นก็กำหนดtotal = total + price_stockขึ้นมาเพราะมันจะมีการลดราคาจากยอดรวมตอนแรกด้วยจากนั้นก็ใช้ if elseในการเช็คพอเช็คเสร้จก็ใช้ round half up ในการปัดเศษและprint f string
```
...
```text
เพื่อตรวจสอบการทำงานของโปรแกรมว่าเงื่อนไขนักเรียนที่เป็นสมาชิกสามารถนำยอดรวมมาคำนวณส่วนลด 5% ได้ถูกต้องหรือไม่
```
...
```text
ตรวจสอบความถูกต้องกรณีไม่เป็นสมาชิกว่าถ้ายอดรวมเท่ากับ500 บาทพอดีเป๊ะโปรแกรมจะยังจับเข้าเงื่อนไขtotal >= 500 เพื่อลด 3% ได้ถูกต้องหรือไม่
```
...
```text
เป็นเคสที่มักพบความผิดพลาดบ่อยที่สุดเพื่อทดสอบว่าโปรแกรมสามารถจัดการการปัดเศษทศนิยมแบบ Round half up ได้จริงโดยใช้เลขที่คำนวณส่วนลดแล้วตกเศษ .025 พอดี
```
