...
```text
Step1:รับค่าa,b,goal มาเป็นint
Step2:หาว่าใช้อิฐก้อนใหญ่มากสุดกี่ก้อน
Step3:หาจำนวนที่เหลือ
Step4:เช็คว่าจำนวนที่เหลือน้อยกว่าเท่ากับaมั้ย
Step5:ใช้elseปริ้น ลบ1
```
...
```text
วิธีสุดท้ายเหมือนกับแผนแรก คือจะรับค่าa,b,goalเข้ามาก่อนจากนั้นก็จะหาว่าใช้อิฐก้อนใหญ่สุดกี่ก้อน โดยเอาgoal //5 แล้วเราก็ต้องใช้minด้วยเพราะสมมติb=2 goal=50 แล้วเราคิดเลขออกมาจะได้10ถ้าเราไม่ใช้min bจะเท่ากับ10ทั้งๆที่เรามีแค่2เราเลยต้องใช้min แล้วก็หาจำนวนที่เหลือโดยการเอาgoal-(big_use *5) แล้วก็เช็คว่าremainน้อยกว่าเท่ากับaมั้ย ถ้าใช่ก็ปริ้นremainเลย ถ้าไม่ตรงกับกรณีไหนเลยก็ปริ้น-1
```
...
```text
ตรวจสอบการทำงานแบบทั่วไปที่ต้องใช้อิฐทั้งสองขนาดผสมกัน โดยโปรแกรมจะต้องคำนวณหักลบอิฐก้อนใหญ่ที่ใช้ได้สูงสุดออกก่อน แล้วจึงเช็คว่ามีอิฐก้อนเล็กเพียงพอสำหรับเศษที่เหลือหรือไม่
```
...
```text
ตรวจสอบกรณีหลอกที่ "ความยาวรวมของอิฐทั้งหมดมีมากพอ" (2 + 15 = 17 ซึ่งมากกว่า 14) แต่ในความเป็นจริงไม่สามารถสร้างได้ เพราะใช้อิฐใหญ่ได้แค่ 2 ก้อน (เท่ากับ 10) ทำให้เหลือเศษที่ต้องใช้ก้อนเล็กอีก 4 นิ้ว แต่เรามีก้อนเล็กแค่ 2 ก้อน จึงต้องแสดงผลเป็น -1
```
...
```text
ตรวจสอบบั๊กที่มักพบบ่อยเมื่อgoalมีค่าน้อยกว่า 5 โค้ดที่ถูกต้อง (การใช้ ⁠goal // 5⁠) จะต้องคำนวณจำนวนอิฐใหญ่เป็น 0 โดยอัตโนมัติ และหันไปใช้อิฐเล็กเพียงอย่างเดียวให้ครบตามจำนวนเป้าหมาย
```
