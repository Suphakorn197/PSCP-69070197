...
| OJ submission ID, if submitted | 662601 |
| OJ status | Pass |
...
```text
ผมให้ช่วยเรื่องแนวคิดแบบว่า การใช้while loopเพื่อหาแถบสีโดยที่เราต้องลดพื้นที่ลงเรื่อยๆจนไม่มีพื้นที่เหลือ
แล้วก็ให้ช่วยเรื่องdiag = diag + 1 ว่าเป็นยังไงและband = (diag - 1) // L + 1 ทำให้ผมเข้าใจและนึกภาพออก
```
...
```text
ช่วยให้สังเกตเห็นถึงความจำเป็นของการทำการปรับตัวเลขในสูตรคำนวณโดยเฉพาะการต้องเอา (diag - 1) ก่อนนำไปหาร // L เพื่อป้องกัน Edge case ที่ตัวเลขหารลงตัวพอดี เช่นเส้นที่ 3 หารแถบขนาด 3ซึ่งถ้าไม่ลบ 1 ก่อน จะทำให้ตัวเลขล้นไปอยู่แถบถัดไปผิด
```
...
```text
ตรวจสอบความเข้าใจของตัวเองโดยการเขียนคอมเมนต์อธิบายการทำงานของโค้ดทีละบรรทัดใน VS Code และทดสอบคำนวณในกรณีสมมติที่ L=1 และ L=3 เพื่อยืนยันว่าสูตร (diag - 1) // L + 1 ทำงานได้ถูกต้องและไม่เกิดบักขอบเขตตัวเลขล้นแถบ
```
...
```text
ได้เรียนรู้ว่าเครื่องหมายเท่ากับในโปรแกรมมิ่งไม่ใช่สมการคณิตแต่เป็นการอัปเดตค่าทับตัวแปรเดิม เช่นการใช้ diag = diag + 1 นอกจากนี้ยังเข้าใจความสำคัญของการวางลำดับคำสั่งใน while loop ว่าการวางคำสั่งบวกเลขไว้หลังเงื่อนไข break จะช่วยป้องกันไม่ให้ค่าตัวแปรเกินไป 1 ก้าว
```
