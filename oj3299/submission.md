...
```text
Input:
รับค่าL,N โดยใช้.split()และ ผมให้L = int(input[0])คือตอนแรกเราใช้.split()แล้วมันจะเป็นการสร้างlistละผมต้องการให้Lอยู่เป็นตัวแรกในกล่องlistเลยเขียน[0] ค่าNก้เหมือนกันผมเลยให้เป็น[1]
Output:
จำนวนเต็มของแถบของช่องสุดท้ายที่มีการปลูกดอกไม้ ในกรณีที่ปลูกด้วยแถบขนาด L และปลูกไป N ช่อง
Constraints:
เราต้องใช้.split()และมีการใช้while loop
```
...
```text
Step1:รับค่าL,N โดยใช้.split()และ ผมให้L = int(input[0])คือตอนแรกเราใช้.split()แล้วมันจะเป็นการสร้างlistละผมต้องการให้Lอยู่เป็นตัวแรกในกล่องlistเลยเขียน[0] ค่าNก้เหมือนกันผมเลยให้เป็น[1]
Step2:กำหนดให้เส้นทแยงมุม(diag) = 1คือเราจะกำหนดให้เส้นทแยงมุมเริ่มที่1
Step3:เข้า while loop ถ้าN>0
Step4: N = N - diag เราลบdiagเพราะอยากให้พื้นที่ลดลงไปเรื่อยๆ
Step5:if N <= 0: เมื่อพื้นที่เต้มแล้วให้break
                   break
Step6:diag = diag + 1 ก็คือถ้าพื้นที่ยังไม่หมดเส้นทแยงมุมก็จะเพิ่มไปเรื่่อยๆ
Step7:band = (diag - 1) // L + 1 บรรทัดนี้คือการหาแถบ ที่diagต้องลบ1เพราะว่าถ้าสมมติLเป็น3 แล้วถ้าไม่บวก
แถบที่1 = 1//3  = 0, แถบที่2 = 2//3 = 0,แถบที่3 = 3//3 = 1 มันผิดตรงนี้แหละเพราะL3 มันต้องเท่ากันทั้ง3แถบ
สมมติเป็นL1 ถ้าไม่บวก:แถบที่1 = 1//1 = 1+1 = 2,แถบที่2 = 1//2 = 0+1 = 1  มันจะไม่เท่ากัน แต่ถ้าเราลบจะเป็นแบบนี้แถบที่1 = 0//1 = 0 + 1 = 1, แถบที่2 = 1//2 = 0 +1 = 1 ซึ่งมันเท่ากันเราเลยต้องมีการ-1ตรงdiag ส่วนตรงL+1เราต้อง+1เพราะถ้าในคอมปกติจะเป็น0,1,2แต่เราต้องการแถบสีจริงๆที่ไม่เหมือนในคอมเลยต้อง+1
Step8:print(band)
```
...
```text
วิธีสุดท้ายเหมือนกับแผนแรกครับเพราะการลูป while เพื่อหาเส้นทแยงมุมสุดท้าย และการคำนวณแถบด้วย (diag - 1) // L + 1 สามารถแก้ปัญหาและจัดกลุ่มได้ถูกต้องครอบคลุมแล้วโดยไม่ต้องปรับแก้สมการเพิ่มเติมจากแผนเดิม
```
...
```text
ตรวจสอบการทำงานทั่วไป ว่าถ้าดอกไม้มีการล้นไปอยู่แถบสีที่ 2 ระบบสามารถคำนวณและปัดขึ้นได้ถูกต้องมั้ย
```
...
```text
ตรวจสอบกรณีขอบเขตที่ตัวเลขน้อยที่สุดที่เป็นไปได้ตามโจทย์กำหนด คือมีดอกไม้แค่ 1 ดอก และความหนาแถบคือ 1 เพื่อดูว่าโปรแกรมทำงานได้โดยไม่ Error 
```
...
```text
ตรวจสอบกรณีที่เส้นทแยงมุมตกเป็นเส้นสุดท้ายของแถบพอดี เช่น L=3 ดอกไม้หมดที่เส้นทแยงมุม 3 พอดีเพื่อยืนยันว่าสูตร (diag - 1) ของเราทำงานได้เป๊ะ และไม่เผลอปัดตัวเลขล้นไปแถบถัดไป
```
