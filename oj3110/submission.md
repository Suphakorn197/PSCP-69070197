...
```text
Input:
ต้นทาง ปลายทาง และ น้ำหนัก
Output:
ราคาการนําส่งพัสดุที่ได้จากข้อมูลนําเข้า
constraints:
ตอนรับค่าต้นทาง กับ ปลายทางเข้ามา ต้องใช้.upper()ด้วย
```
...
```text
Step1:รับค่าต้นทาง ปลายทาง และน้ำหนักเข้ามา
Step2:กำหนด ค่าธรรมเนียมเริ่มต้น ค่าธรมมเนียมน้ำหนัก และ ผลรวม =0
Step3:ใช้if elif ในการเช็คว่าถ้าต้นทาง และ ปลายทางคืออะไรแแล้วคำนวณ
Step4:ใช้ifเช็คว่าผลรวมมากกว่า0มั้ย ถ้ามากกว่าให้ปริ้นผลรวมออกมาเป็นทศนิยม2ตำแหน่ง
```
...
```text
วิธีสุดท้ายเหมือนกับแผนแรกเลย ก็คือรับค่าต้นทางและปลายทางมาเป็นstrโดยต้องใช้.upper()เพราะโจทย์บอกว่าต้องเป็นตัวพิมพ์ใหญ่ และ รับค่าน้ำหนัก จากนั้นก็กำหนดค่าธรรมเนียมเริ่มต้น,ค่าธรรมเนียมน้ำหนัก และ ผลรวม =0 เพราะเราต้องคำนวณ จากนั้นก็ใช้if elifในการเช็คว่าถ้าต้นทางเป็นตัวนี้ และปลางทางเป็นตัวนี้ ค่าธรรมเนียมเริ่มต้นและค่าธรรมเนียมน้ำหนักเป็นเท่าไหร่แล้วก็เอาน้ำหนักไปคูณค่าธรรมเนียมน้ำหนักและไปบวกค่าธรรมเนียมเริ่มต้น แล้วก็จะได้totalออกมา แล้วก็เช็คให้หมดจนครบตารางแล้วก็ใช้elseในกรณีที่เส้นทางไม่ระบุในตารางให้ปริ้นออกมาเป็นError และใช้ifเช็คว่าtotalมากกว่า0มั้ย ถ้ามากกว่า0ให้ปริ้นออกมาเป็นทศนิยมสองตำแหน่ง
```
...
```text
Weightรับค่าเป็นfloatเผื่อตอนรับค่าเข้ามาน้ำหนักจะเป็นเลขทศนิมยม
```
...
```text
ในกรณีที่มันไม่อยู่ในเส้นทางจะปริ้นออกมาเป็นError
```
...
```text
ในกรณีที่ น้ำหนักที่รับเข้ามาเป็น0จะแสดงผลออกมาแค่ค่าธรรมเนียมเริ่มต้น และ ปริ้นออกมาเป็นทศนิทยม2ตำแหน่ง
```
