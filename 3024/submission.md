...
```text
Input:
รับค่าคะแนนรวมทั้งสามคนและคะแนนสูงสุดของทั้งสามคน
Output:
แสดงผลลัพธ์ออกมาเป็น "Surprising" หรือ "Not surprising"
Constraints:
คะแนนที่โหวตเป็นจำนวนจริงไม่ใช่จำนวนเต็ม
```
...
```text
Step 1: รับค่าคะแนนรวมและคะแนนสูงสุดเป็นจำนวนจริง
Step 2: คำนวณหาคะแนนต่ำสุดที่เป็นไปได้โดยเอาคะแนนรวมลบด้วยคะแนนสูงสุด 2 ครั้ง
Step 3: ตรวจสอบว่าmin_scoreติดลบหรือไม่ถ้าติดลบให้กำหนดค่าmin_score = 0
Step 4: ตรวจสอบเงื่อนไขว่าผลต่างของคะแนนสูงสุดกับคะแนนต่ำสุดมากกว่า 2 หรือไม่ 
Step 5: ถ้าเงื่อนไขเป็นจริงให้พิมพ์ "Surprising" แต่ถ้าไม่จริงให้พิมพ์ "Not surprising"
...
```text
วิธีสุดท้ายเหมือนกับแผนแรก จะรับค่าคะแนนสูงสุดและคะแนนรวม แล้วคำนวณหาคะแนนต่ำสุดที่เป็นไปได้โดยเอาคะแนนรวมลบด้วยคะแนนสูงสุด 2 ครั้ง ตรวจสอบว่าmin_scoreติดลบหรือไม่ถ้าติดลบให้กำหนดค่าmin_score = 0
และตรวจสอบเงื่อนไขว่าผลต่างของคะแนนสูงสุดกับคะแนนต่ำสุดมากกว่า 2 มั้ยถ้าเงื่อนไขเป็นจริงให้พิมพ์ "Surprising" แต่ถ้าไม่จริงให้พิมพ์ "Not surprising"
```
...
```text
ทดสอบกรณีทั่วไปที่คะแนนเกาะกลุ่มกันคำนวณแล้วผลต่างไม่เกิน 2
```
...
```text
ทดสอบกรณีที่ผู้ใช้ป้อนค่าเป็นเลขทศนิยมตามที่โจทย์ระบุและเช็คเงื่อนไขกรณีที่ผลต่างเกิน 2
```
...
```text
ทดสอบEdge Caseที่พบบ่อยคือกรณีที่สมการคำนวณคะแนนต่ำสุดแล้วได้ค่าติดลบเพื่อเช็คว่าโค้ดบรรทัด if min_score < 0: สามารถดักจับและปรับค่าให้กลายเป็น 0 ได้ถูกต้อง
```
