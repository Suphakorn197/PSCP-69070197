...
```text
Input:
รับค่าเดือนเป็นจำนวนเต็มและวันที่เป็นจำนวนเต็ม
Output:
ออกมาเป็นฤดูจากวันที่ ที่รับค่าเข้าไป
Constraints:
ข้อมูลเดือนที่รับเข้ามา จะต้องเป็นจำนวนเต็มตั้งแต่ 1 ถึง 12 
```
...
```text
Step 1: รับค่าข้อมูลเข้ามาเป็นตัวเลขจำนวนเต็ม2ค่า คือเดือนและวันที่
Step 2: กำหนดค่าคงที่สำหรับฤดูทั้ง 4 
Step 3: ใช้คำสั่งเงื่อนไขif,elifเพื่อตรวจสอบว่าเดือนและวันที่รับเข้ามาตกอยู่ในช่วงเวลาของฤดูไหน 
Step 4:แสดงผลลัพธ์ โดยพิมพ์ชื่อฤดูกาลที่ผ่านเงื่อนไขในStep 3 
```
...
```text
วืธีสุดท้ายเหมือนกับแผนแรกครับ ก็คือรับค่าข้อมูลวันที่กับเดือนมาและใช้เงื่อไนif elifเพื่อตรวจสอบว่าวันที่และเดือนที่รับเข้ามาจะเป็นฤดูอะไร
```
...
```text
เพื่อตรวจสอบการทำงานในกรณีปกติที่เดือนและวันที่อยู่กลางฤดูกาลพอดีไม่มีเงื่อนไขที่ซับซ้อน
```
...
```text
เพื่อตรวจสอบเงื่อนไขตอนเปลี่ยนฤดูว่าโค้ดใช้เครื่องหมาย >= 21 ได้อย่างถูกต้องและขยับไปเป็นฤดูใหม่มั้ย
```
...
```text
เพื่อตรวจสอบกรณีวันสุดท้ายก่อนที่จะเปลี่ยนฤดูว่าโปรแกรมยังคงแสดงผลเป็นฤดูเดิมอยู่มั้ย
```
