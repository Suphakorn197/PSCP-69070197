...
```text
Input:
จะรับค่าname และเขียน.strip()เผื่อมีช่องว่างและ.lower()เพราะโจทย์ต้องการให้เป็นตัวพิมพ์เล็ก
Output:
จะออกมาเป็นการสลับชื่อจากหลังมาหน้า
Constraints:
ข้อมูลชื่อที่รับเข้ามาอาจมีช่องว่างส่วนเกินติดมาด้วยและอาจมีตัวอักษรพิมพ์ใหญ่ผสมพิมพ์เล็กเลยต้องเขียนคำสั่ง.strip()และ.lower()ก่อนทำการสลับตำแหน่ง
```
...
```text
Step 1: รับค่าข้อความเก็บไว้ในตัวแปรname
Step 2: ทำการใช้.srtip()เพื่อให้ชัวว่าไม่มีช่องว่างและใช้.lower()เพื่อให้เป็นตัวพิมพ์เล็ก
Step 3: ใช้การSlicingเพื่อที่จะทำให้ข้อความสลับตัวอักษรจากหลังมาหน้า
Step 4: printแล้วดูข้อความที่ถูกสลับแล้วออกทางหน้าจอ
ในตอนแรกนึกว่าต้องใช้reverseแต่จริงๆreverseมันใช้คนละแบบเลยถามพี่TAแล้วพี่TAก็ใบ้มาว่าต้องใช้อะไรแต่จำไม่ได้เลยถามเพื่อนทำให้รู้ว่าต้องใช้slicing
```
...
```text
วิธีสุดท้ายเหมือนกับแผนแรกครับคือผมรับค่าชื่อใช้.strip() และ.lower()ให้เรียบร้อยก่อนจากนั้นใช้Slicing ในการสลับข้อความจากหลังมาหน้าซึ่งวิธีนี้ใช้งานได้จริงและแก้ปัญหาได้ตรงจุดตามที่เพื่อนและพี่TAแนะนำครับ
```
...
```text
เพื่อที่จะดูว่าถ้าตอนที่รับinputมาแล้วเผลอมีช่องว่างแบบเราเผลอspacebarแบบจะไม่ได้ตั้งใจตอนoutputออกมาช่องว่างนั้นจะหายมั้ย
```
...
```text
เพื่อที่จะดูว่าถ้าใช้.lower() ถ้าตอนรับค่าเข้ามามี ตัวพิมพ์ใหญ่ตอนoutputออกมาจะเป็นตัวพิมพ์เล็กมั้ย
```
...
```text
เพื่อที่จะดูว่าการใช้slicingที่ผมใช้มันถูกต้องมั้ยแบบการใช้slicingมีถึง3แบบ แล้ววิธีที่ผมใช้มันถูกมั้ย[::-1]
```
...
```text
ได้มาบอกว่าต้องใช้slicing
```

คุณยังทำอะไรด้วยตนเอง

```text
ได้ไปหาว่าslicingใช้ยังไงและเขียนด้วยตัวเอง
```
