...
```text
Input:
เป็นการรับค่าของสีที่1และสีที่2
Output:
ออกมาเป็นผลลัพธ์หลังจากที่เกิดการผสมสีแล้ว
Constraints:
สีที่รับเข้ามาอาจมีการสลับตำแหน่งกันได้ เช่น แดงผสมเหลือง หรือ เหลืองผสมแดง โปรแกรมจึงต้องมีการเช็คเงื่อนไขสีที่สลับที่กันได้และต้องคอยระวังตัวพิมพ์ใหญ่พิมพ์เล็ก
```
...
```text
Step1:รับค่าตัวแปรสีที่ 1และสีที่2
Step2: ต้องมีการสร้างตัวแปรcolorที่เกิดจากการผสมสี2สีเพื่อความง่ายในการเขียนโค้ด
Step3:ตอนเขียน if,elif ดูว่าสีต้องมีการสลับกัน
Step4:ต้องเขียนกรณีที่สีเดียวกันผสมกันด้วย

ตอนแรกผมลืมกรณีที่ต้องสลับสีกันผมก็งงว่าทำไมไม่ถูกจนให้แชทเช็คให้ละมันบอกต้องสลับด้วย พอเสร็จปั้ปก็ไม่รู้ว่าต้องมีกรณีที่สีเดียวกันผสมกันพอไปถามแชทผมถึงกับอ๋อเลยว่าต้องมีกรณีนี้ด้วย
```
...
```text
วิธีสุดท้ายเหมือนกับแผนแรกผมใช้การรวมตัวแปรสีเข้าด้วยกันแล้วใช้เงื่อนไขif-elif เช็คผลลัพธ์โดยไม่ลืมกรณีอย่างการสลับที่สีและกรณีการใส่สีเดียวกันวิธีนี้แก้ปัญหาได้ตรงจุดและใช้งานได้จริง
```
...
```text
เพื่อดูว่าการทำงานของโปรแกรมว่าสามารถประมวลผลการผสมสีสองสีที่ต่างกันตามเงื่อนไขได้ถูกต้องมั้ย
```
...
```text
เป็นกรณีที่ผมเคยพลาดมาก่อนจึงเลือกมาทดสอบเพื่อที่จะเช็คว่าถ้าป้อนลำดับสีสลับกันโปรแกรมจะยังประมวลผลได้ถูกต้องมั้ย
```
...
```text
เพื่อเช็คว่ามีกรณีที่ผู้ใช้ป้อนสีเดียวกันทั้งสีที่1และสีที่2เพื่อที่จะแสดงผลลัพธ์ออกมาเป็นสีเดิม
```
