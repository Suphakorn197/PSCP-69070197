...
| OJ submission ID, if submitted | 656659 |
| OJ status | Pass |
...
```text
ให้ช่วยตรงบรรทัด  if len(text) > max_len: คือตอนแรกผมนึกไม่ถึงว่าเราต้องใช้ lenเพื่อหาตัวสูงสุดเพื่อที่จะได้รู้ว่า * ต้องมีกี่ตัว และ บรรทัด  box_width = max_len + 4 คือตอนแรกผมนึกไม่ออกว่าต้องมีบรรทัดนี้เพราะดูไม่ออกว่า ต้องบวก4 ด้วย และให้ช่วยเรื่อง w:<{max_len} คือผมจำไม่ได้ว่าการใช้ < คือจะทำให้ชิดซ้าย 
```
...
```text
แชทช่วยให้สังเกตว่าความกว้างรวมของกรอบไม่สามารถใช้แค่ความยาวข้อความได้แต่ต้องบวกพื้นที่เผื่อสำหรับตัวดาว 2 ฝั่งและช่องว่าง 2 ฝั่งเสมอก็คือตรง+4นอกจากนี้ยังช่วยให้สังเกตเห็นเทคนิคการใช้f-string (<) ว่าสามารถจัดการเรื่องการชิดซ้ายและเติมช่องว่างที่เหลือให้เต็มกรอบได้อัตโนมัติโดยไม่ต้องเขียนลูปต่อข้อความเอง
```
...
```text
ได้นำความเข้าใจเรื่องf-string (<)และสูตรคำนวณความกว้าง max_len + 4 ไปปรับแก้ในโค้ดของตัวเอง
```
...
```text
ได้เรียนรู้ว่าการคำนวณความกว้างของขอบจะต้องเผื่อพื้นที่ว่างซ้ายขวาและตัวอักษรกรอบเสมอไม่สามารถอิงแค่ความยาวสูงสุดของข้อความเพียงอย่างเดียวได้นอกจากนี้ได้เรียนรู้เทคนิคการใช้ f-stringร่วมกับ< เพื่อจัดข้อความให้ชิดซ้ายและเติมช่องว่างที่เหลือให้อัตโนมัติ
```
