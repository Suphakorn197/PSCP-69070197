...
จำนวนเต็ม x
output:
จำนวนกำแพงน้อยที่สุดที่จะต้องพังเพื่อเดินทางจากห้อง N ไปจนถึงห้องหมายเลข 1 ถ้าไม่ต้องพังกำแพงเลย ให้ตอบ 0
Constraints:
ต้องใช้while
```
...
```text
Step 1: รับหมายเลขห้อง x
Step 2: ดูรูปแบบการเรียงหมายเลขห้องเพื่อหาว่า x อยู่ชั้นไหน
Step 3: หาตำแหน่งของ x ภายในชั้นนั้น
Step 4: วิเคราะห์จากรูปว่าห้องตำแหน่งนั้นสามารถขึ้นไปชั้นถัดไปได้อย่างไร
Step 5: คำนวณจำนวนกำแพงที่ต้องพัง และหาค่าที่น้อยที่สุด
Step 6:แสดงจำนวนกำแพงที่น้อยที่สุด
```
...
```text
วิธีสุดท้ายที่ใช้จริงคือหาชั้นของห้องxก่อนโดยใช้ floor^2 แล้วใช้ while เพื่อเพิ่มชั้นไปเรื่อย ๆ จนเจอชั้นที่มีห้องxจากนั้นหาค่า far เพื่อดูว่าห้องxอยู่ตำแหน่งไหนของชั้น และตรวจสอบค่าเป็นเลขคู่หรือเลขคี่เพื่อคำนวณจำนวนกำแพงที่ต้องพัง
```
...
```text
ตรวจสอบค่าพิเศษที่เล็กที่สุด เพราะห้องหมายเลข 1 อยู่ที่จุดหมายอยู่แล้ว จึงไม่ต้องพังกำแพง
```
...
```text
ตรวจสอบกรณีทั่วไปที่อยู่ชั้นที่ 5 และเป็นห้องที่ต้องพังกำแพงหลายกำแพง เพื่อดูว่าการคำนวณจำนวนกำแพงทำงานถูกต้อง
```
...
```text
ตรวจสอบกรณีที่ N เป็นเลขกำลังสอง ซึ่งเป็นห้องสุดท้ายของแต่ละชั้น เช่น 4, 9, 16, 25 เพื่อดูว่าโปรแกรมหาชั้นได้ถูกต้อง
```
