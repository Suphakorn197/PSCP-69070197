...
```text
Step1:รับค่าเป็นผลการออกรางวัลสลากกินแบ่ง และ หมวดตัวอักษร และ หมายเลขสลากที่กระต่ายน้อยซื้อ
Step2:ใช้if elif ในการเช็คว่าถ้าตรงกี่ตัวตรงปริ้นออกมาเป็นกี่บาท
```
...
```text
วิธีสุดท้ายเหมือนกับแผนแรก โดยที่จะรับค่ารับค่าเป็นผลการออกรางวัลสลากกินแบ่ง และ หมวดตัวอักษร และ หมายเลขสลากที่กระต่ายน้อยซื้อ และตรงหมายเลขเราไม่ต้องให้มันเป็นintเพราะข้อนี้ต้องใช้slicingและslicingหั่นได้แค่strเท่านั้น แล้วก็ใช้if elif elseในการเช็คให้ครบทุกกรณี
```
...
```text
เพื่อทดสอบการจัดลำดับเงื่อนไขกรณีที่ผู้ใช้ถูกทั้งเลขท้าย 3 ตัวและ2 ตัวพร้อมกันโปรแกรมจะต้องเลือกจ่ายเงินรางวัลที่ใหญ่กว่าเสมอ โดยไม่เผลอหยุดทำงานที่เงื่อนไขถูก 2 ตัว
```
...
```text
เพื่อทดสอบ 2 เรื่องพร้อมกัน 1.ระบบสามารถรับมือกับinputที่เป็นตัวพิมพ์เล็กได้และ2.ทดสอบว่าตัวเลขไม่ถูกแปลงเป็นint()เพราะถ้าแปลง เลขศูนย์ที่อยู่ข้างหน้าจะหายไป
```
...
```text
ทดสอบกรณีทั่วไปสำหรับรางวัลที่เล็กที่สุดเพื่อเช็คเงื่อนไขหมวดตัวอักษรตรงกัน แต่ตัวเลขไม่มีส่วนไหนตรงกันเลยทั้ง 2 ตัวท้ายและ 3 ตัวท้าย
```
