...
```text
Input: 
รับค่าX,Y เป็น int และใช้.split()
Output:
...
```text
Step1:รับค่าX,Y เป็น int และใช้.split()
Step2:สร้างcurrent_position = 0และcount = 0
Step3:กำหนดjump_power = X
Step4:เข้าสู่while loop
Step5:ใช้ifและออกด้วยbreak
Step6:ใช้elseเผื่อในกรณีที่ไม่ถึง
```
...
```text
วิธีสุดท้ายเหมือนกับแผนแรก คือรับค่าX,Y เป็น int และใช้.split() และสร้างcurrent_position = 0และcount = 0 เรงต้องสร้างเพราะคอยเช็คว่ากระโดดไปที่เท่าไหร่แล้วและเช็คว่าโดดไปกี่ครั้งแล้ว แล้วก็กำหนดjump_power = X แล้วก็เข้าสู้ while loop เราต้องใช้while loopเพราะโจทย์ไม่ได้กำหนดมาว่าต้องโดดกี่ครั้งแต่ให้เราหา แล้วถ้าjump_power > 0  ตำแหน่งก็จะเพิ่มทีละXเมตร แล้วก็จะนับจำนวนการโดดด้วย จากนั้นก็เช็คว่าตำแหน่งที่อยู่มากกว่าเท่ากับYรึยัง ถ้าใช่แล้วก็print(count)ได้เลย แล้วก็ออกจากwhile loopโดยการใช้break จากนั้นก็เขียน  jump_power -= 2 เพราะจะลบไปทีละ2เมตร และ else: print(-1) ในกรณีที่ไม่ถึง
```
...
```text
เพื่อเช็คว่าลูปการคำนวณปกติทำงานได้ถูกต้องไหม
```
...
```text
เพื่อเช็คว่าถ้ากบไม่ต้องกระโดดเลย หรือกระโดดทีเดียวถึง จะทำงานอย่างไร
```
...
```text
เพื่อเช็คว่าส่วน else หรือเงื่อนไข -1 ของเราทำงานถูกต้องไหม เมื่อแรงหมดแล้วยังไม่ถึงเป้าหมาย
```
