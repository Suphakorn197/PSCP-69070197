...
```text
Input:
รับค่าเข้ามาเป็นstrและใช้.upper()
Output:
แสดงผลตามชื่อของไพ่
Constraints:
ต้องใช้.upper()ด้วย เพราะถ้าไม่ใช้ มันจะเกิด errorได้สมมติ รับเข้ามาเป็น Qs แต่ว่าต้องเป็น S  ถึงจะเป็น spades
```
...
```text
Step1:รับค่าเข้ามาเป็นstrและใช้.upper()
Step2:ใช้slicing คือเราจะเอาแค่จะแค่ทุกตัวยกเว้นตัวท้าย กับ ตัวท้าย ก็คือ [:-1] [-1]
Step3:สร้างกล่องเปล่าขึั้นมา
Step4:เช็คว่าถ้า เป็นตัวอักษรอะไรเราต้องได้ชื่อไหนออกมา
STep5:print f string
```
...
```text
วิธีสุดท้ายเหมือนกับแผนแรก คือรับค่าเข้ามาเป็นstr แล้วก็ใช้.upper()  เพราะถ้าไม่ใช้ มันจะเกิด errorได้สมมติ รับเข้ามาเป็น Qs แต่ว่าต้องเป็น S  ถึงจะเป็น spades แล้วก็ต้องสร้างกล่องเปล่าขึ้นมา ถ้าไม่สร้างจะเกิดerrorได้แล้วก็เช็คว่าถ้าเป็นตัวอักษรนี้จะปริ้นออกมาเป็นอะไร แล้วก็print f string
```
...
```text
ทดสอบกรณีปกติ เพื่อเช็คการทำงานของโปรแกรมว่าสามารถแยกแต้มไพ่กับดอกไพ่ที่มีความยาว 2 ตัวอักษร และแปลเป็นคำเต็มได้ถูกต้องตามเงื่อนไข
```
...
```text
ดสอบค่าพิเศษที่ความยาวของInputเป็น3ตัวอักษรเลข 10เพื่อเช็คว่าคำสั่งSlicing [:-1]สามารถดึงเลข10ออกมาได้ครบถ้วนโดยไม่ขาดหาย และเช็คว่าเงื่อนไขelseสามารถทำงานเพื่อปริ้นท์ตัวเลขออกมาได้ถูกต้อง
```
...
```text
ทดสอบข้อผิดพลาดโดยจำลองสถานการณ์ที่ผู้ใช้ป้อนตัวพิมพ์เล็กหรือพิมพ์เล็กผสมพิมพ์ใหญ่เพื่อเช็คว่าโปรแกรมมีการใช้คำสั่ง .upper() เพื่อดักจับและแปลงค่าให้เป็นมาตรฐานเดียวกันก่อนนำไปประมวลผลหรือไม่
```
