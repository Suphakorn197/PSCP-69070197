...
จํานวนนับ r 
Output:
จำนวนตัวเลขที่บอกว่าตั้งแต่ช่วงAถึงBมีตัวเลขอยู่กี่ตัวที่หารด้วยdแล้วเหลือเศษr
Constraints:
ต้องใช้intเพราะเขาบอกว่าเป็นจำนวนนับ
```
...
```text
Step1: รับค่าตัวเลข A,B,d,r
Step2 : กำหนด count = 0
Step3: ใช้ for loop
Step4: ใช้if เพื่อหาเช็คว่าตัวเลขที่หารด้วยdจะได้เศษrมั้ย
Step5:ใช้count+=1 เพื่อที่จะบวกเข้าไป1ตัวเมื่อเจอตัวที่สามารถหารด้วยdแล้วได้เศษr
Step6 : print
```
...
```text
วิธีสุดท้ายของผมเหมือนกับแผนแรก คือผมรับค่าตัวเลขก่อนแล้วกำหนดcount = 0 เพราะว่าตอนแรกเราไม่มีไรเลยแล้วค่อยไปบวกกับ count+=1ทีหลัง แล้วใช้ for loop เพราะโจทย์ให้หาในช่วงAถึงB แล้วการใช้for loop เป็นสิ่งที่ง่ายที่สุดแล้วการใช้if elseก็ไม่ได้เลยเพราะเราไม่รู้ว่าเลขที่เขาจะให้ในตัวแปรAกับBมาจะเยอะแค่ไหน แล้วก็ใช้ifเช็คว่ามีตัวที่หารdแล้วได้เศษrมั้ย
```
...
```text
เพื่อที่จะทดสอบว่าถ้าเราต้องใช้intเพราะโจทย์บอกว่าเป็นจำนวนนับ
```
...
```text
เพื่อที่จะทดสอบว่าถ้าเราต้อง
กำหนด count =0 ไว้ก่อนในตอนแรกผลลัพธ์เลยจะไม่error
```
...
```text
เพื่อที่จะทดสอบว่า Bในวงเล็บต้อง+1  outputจะออกมาน้อยกว่าที่ต้องการเพราะ ในคำสั่งfor loop เวลาจบมันจะจบที่เลขก่อนหน้า
```
