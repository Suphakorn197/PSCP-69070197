...
```text
Input:
ค่าN,K,Tโดยต้องใช้ map .split()ด้วย
Output:
...
```text
Step1:รับค่าN,K,T เข้ามาเป็นintและต้องใช้mapกับ.split()ด้วย
Step2:เช็คว่าถ้าเจอขโมยหรือก็คือTเป็นคนแรกเลย ให้print(1)เลย
Step3:ใช้else: ก็คือถ้าT ไม่เท่ากับ1 ให้ count และ current = 1 countคือคนที่ได้พิจารณา currerntคือตำแหน่ง
Step4:ใช้ while true เพราะมันเป็นอนันต์
Step5:ให้current += K เพราะตำแหน่งจะขยับบวกเพิ่มไปทีละ K เวลาวนลูปแต่ละรอบ
Step6:ใช้ifเช็คว่ามากกว่าจำนวนNมั้ย ถ้ามากกว่าต้องลบด้วยNด้วย
Step7:เช็คว่า current = 1ยังก็คือวนครบรอบยัง ถ้าครบแล้วให้breakเลย
Step8:เช็คว่า current = T มั้ยถ้าอยู่ตรงคนขโมย ให้count += 1 ก็คือให้นับคนที่พิจารณาด้วยจากนั้นก็break
Step9: count+= 1 ก็คือคนปกติในวงถ้าไม่ใช่โจรจะบวกเข้าไป
Step10:print(count)
```
...
```text
วิธีสุดท้ายเหมือนกับแผนแรกคือจะรับค่าN,K,T เข้ามาเป็นintและต้องใช้mapกับ.split()ด้วยจากนั้นเช็คว่าถ้าเจอขโมยหรือก็คือTเป็นคนแรกเลย ให้print(1)เลยแล้วก็ใช้else ก็คือถ้าT ไม่เท่ากับ1 ให้ count และ current = 1 countคือคนที่ได้พิจารณา currerntคือตำแหน่งแล้วก็ใช้ while true เพราะมันเป็นอนันต์แล้วก็ให้current += K เพราะว่าตำแหน่งจะขยับบวกเพิ่มไปทีละ K เวลาวนลูปแต่ละรอบแล้วก็ใช้ifเช็คว่ามากกว่าจำนวนNมั้ย ถ้ามากกว่าต้องลบด้วยNด้วยแล้วเช็คว่า current = 1ยังก็คือวนครบรอบยัง ถ้าครบแล้วให้breakเลยจากนั้นเช็คว่า current = T มั้ยถ้าอยู่ตรงคนขโมย ให้count += 1 ก็คือให้นับคนที่พิจารณาด้วยจากนั้นก็breakแล้วก็count+= 1 ก็คือคนปกติในวงถ้าไม่ใช่โจรจะบวกเข้าไปแล้วก็print(count)
```
...
```text
ตรวจสอบการทำงานแบบปกติที่มีการวนรอบและของขวัญวนกลับมาจบที่คนที่ 1 โดยข้ามโจรไป
```
...
```text
ตรวจสอบเมื่อคนที่ 1เป็นโจรT = 1 โปรแกรมควรจะหยุดทำงานทันทีตั้งแต่เริ่มโดยไม่ต้องส่งของขวัญต่อ
```
...
```text
ตรวจสอบกรณีที่ของขวัญวนกลับมาหาคนที่ 1 อย่างรวดเร็วก็คือขยับทีละครึ่งวงและกระโดดข้ามโจรไปเลยเพื่อดูว่าลูป while หยุดทำงานถูกจังหวะหรือไม่
```
