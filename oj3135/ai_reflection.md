...
| OJ submission ID, if submitted | 641720 |
| OJ status | Pass |
...
```text
ให้แชทช่วยอธิบายการใช้ while loopและช่วยหาบัคกรณีที่ T=1,
```
...
```text
แชทช่วยให้สังเกตเห็น กรณีที่โจรคือคนที่ 1,T=1 ซึ่งเป็นเงื่อนไขที่ขาดไปและต้องดักเช็คด้วย if ไว้ตั้งแต่แรกก่อนเข้าลูป 
```
...
```text
ตรวจสอบความเข้าใจการทำงานของลูปด้วยตัวเอง และ เช็คกรณีที่T = 1
```
...
```text
ได้เรียนรู้วิธีการเขียนwhile loopควบคู่กับคำสั่ง break เพื่อจัดการกับปัญหาการวนรอบเป็นวงกลมโดยที่ไม่รู้จำนวนรอบและได้คิดถึงกรณีนอกขอบเขตกรณีที่คนที่ 1 เป็นโจร
```
